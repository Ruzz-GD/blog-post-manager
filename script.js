...
const animateText = () => {
    let magic = phrases[currentIndex];
    
    // Phase 1: Disappearing effect
    for (let i = 0; i <= magic.length; i++) {
        setTimeout(() => {
            hello.textContent = magic.slice(0, magic.length - i);
        }, i * 250);
    }

    setTimeout(() => {
        currentIndex = (currentIndex + 1) % phrases.length;
        magic = phrases[currentIndex]; 
        for (let i = 0; i <= magic.length; i++) {
            setTimeout(() => {
                hello.textContent = magic.slice(0, i);
            }, i * 250);
        }
    }, magic.length * 250);
};

setInterval(animateText, phrases[0].length * 500 + 1000); 